...
function instrumentExpression(expression: string): string {
  ...
  if (comparison) {
    ...
    const leftRef = escapeForSingleQuotedLiteral(comparison.left.trim());
    const rightRef = escapeForSingleQuotedLiteral(comparison.right.trim());
    return `__compare(${left}, '${comparison.operator}', ${right}, '${leftRef}', '${rightRef}')`;
  }
