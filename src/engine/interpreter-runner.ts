...
export class InterpreterRunner {
  ...
  private installRuntime(interpreter: Interpreter, globalObject: unknown): void {
    ...
    bind('__compare', (left, operator, right, leftRef, rightRef) => {
      ...
      const nativeLeftRef = leftRef === undefined ? undefined : String(interpreter.pseudoToNative(leftRef));
      const nativeRightRef = rightRef === undefined ? undefined : String(interpreter.pseudoToNative(rightRef));
      ...
      this.eventQueue.push({
        ...
        right: nativeRight,
        result,
        leftRef: nativeLeftRef,
        rightRef: nativeRightRef,
      });
      return result;
    });
